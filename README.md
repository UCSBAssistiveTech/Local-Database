...
- make a venv, while in the root directory do `python3 -m venv env`
- then do `source env/bin/activate` (this is different per OS, so search up what works for u)
- `pip install -r requirements.txt` to install all the packages
- do `python3 main.py` from the root directory, then the fastapi server is up for (for only rds for now)
- there is now a bunch of requests u can make to the endpoint and it will interact with the postgresql db u have running

- nothing else is ready for now, u can play around with the endpoints by curling it or using something like postman, will make little web interface later
